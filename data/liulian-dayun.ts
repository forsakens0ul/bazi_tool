...
import {
  ...
  generateDayunColumns,
  analyzeLiunianWithDayun,
  BirthInfo as DayunBirthInfo,
} from "./dayun-calculator";

// 天干地支基础数据
export const tianganList = [
  "甲",
  "乙",
  "丙",
  "丁",
...
export const calculateDayun = (birthInfo: any) => {
  ...
  const yearZhi = dizhiList[yearZhiIndex];

  // 月柱（简化版，实际应考虑节气）
  const baseMonthGan = (yearGanIndex % 5) * 2;
  const monthZhiIndex = (month + 1) % 12; // 寅月为正月
  const monthGanIndex = (baseMonthGan + month - 1) % 10;
  const monthGan = tianganList[monthGanIndex];
  const monthZhi = dizhiList[monthZhiIndex];

  // 日柱（简化版）
  const dayGanIndex = (year * 5 + Math.floor(year / 4) + day) % 10;
  const dayZhiIndex = (year * 5 + Math.floor(year / 4) + day) % 12;
  const dayGan = tianganList[dayGanIndex];
  const dayZhi = dizhiList[dayZhiIndex];

  // 时柱
  const timeZhiIndex = hour % 12;
  const baseTimeGan = (dayGanIndex % 5) * 2;
  const timeGanIndex = (baseTimeGan + Math.floor(hour / 2)) % 10;
  const timeGan = tianganList[timeGanIndex];
  const timeZhi = dizhiList[timeZhiIndex];

  // 八字
  const bazi = `${yearGan}${yearZhi} ${monthGan}${monthZhi} ${dayGan}${dayZhi} ${timeGan}${timeZhi}`;

  // 使用新的大运计算逻辑
  const dayunBirthInfo: DayunBirthInfo = {
    year,
    month,
    day,
    hour,
  ...
  if (
    ...
    // 调整起运年龄为1岁4个月
    const adjustedDayunInfo = dayunInfo.map((info, index) => {
      return {
        ...info,
        startAge: 1.4 + index * 10,
        endAge: 1.4 + (index + 1) * 10 - 0.1,
        startYear: year + Math.floor(1.4) + index * 10,
        endYear: year + Math.floor(1.4) + (index + 1) * 10 - 1,
      };
    });

    return adjustedDayunInfo;
  }

  // 一般情况使用新的计算逻辑
  return calculateDayunInfo(dayunBirthInfo, bazi, dayGan, monthColumn);
};
...
export const analyzeLiunian = (year: number) => {
  ...
  return {
    ...
    dizhiRelation: "同气", // 简化
    wuxingFlow: `${getTianganWuxing(tiangan)}${getDizhiWuxing(
      dizhi
    )}相配，气机流通`,
...
// 测试函数
export const testDayunCalculation = () => {
  const testBirthInfo = {
    year: 1990,
    month: 6,
    day: 15,
  ...
  console.log("测试1990年6月15日男性大运计算：");
  const result = calculateDayun(testBirthInfo);

  console.log(
    "年干索引:",
    (1990 - 4) % 10,
    "对应天干:",
    tianganList[(1990 - 4) % 10]
  );
  console.log("是否阳年:", ((1990 - 4) % 10) % 2 === 0);
  console.log(
    "排盘方向:",
    ((1990 - 4) % 10) % 2 === 0 && testBirthInfo.gender === "男"
      ? "顺排"
      : "逆排"
  );

  console.log("大运列表:");
  result.forEach((dayun, index) => {
    console.log(`${dayun.startAge}-${dayun.endAge}岁: ${dayun.ganzhi}`);
  });
